...
async function startServer() {
  ...
  app.delete('/api/products/:id', (req, res) => {
    ...
    saveDb();
    deleteProductFromFirebase(req.params.id);
  ...
  app.post('/api/sellers/:id/test-storage', (req, res) => {
    ...
    try {
      ...
      if (storageType === 'firebase' || storageType === 'supabase') {
        const projectId = creds.projectId || FIREBASE_PROJECT_ID;
        return res.json({ 
          success: true, 
          message: `Successfully connected to Firebase Firestore & Cloud Storage: "${projectId}" for vendor "${seller.storeName}"!` 
        });
